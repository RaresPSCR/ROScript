 ...
 * @note This class is used to store the tokens generated by the lexer and to split them into lines for easier parsing, and assigning them to each Token for easier error reporting.
 */
...
public:
    ...
    void init() {
		/**
		 * @brief Initializes the TokenStream by splitting the tokens into lines and then assigning them to each token.
		 * @note This function is called after the tokens are generated by the lexer. It splits the tokens into lines based on the "NLINE" token, and assigns them to each token for easier error reporting
		 * @returns Tokens vector containing the tokens with their attributes.
		 */
        vector<pair<string, string>> current_line_tokens;
        ...
        // handle last line (no trailing NLINE)
